...
class WeightEstimatorCD:
    """
    Class to estimate weights using coordinate descent.

    Idea: compute gradients as usual
    Only update weight with highest gradient
    ...
    def get_grad_reg(self, graph):
        ...
        for source in graph.nodes():
            for _, target, data in graph.out_edges(source, data=True):
                grad_reg[(source, target)] = (
                    2 * data["weight"] * self.lam2 + self.lam1
                ) * (1 if graph[source][target]["weight"] > 0 else 0)
        return grad_reg
